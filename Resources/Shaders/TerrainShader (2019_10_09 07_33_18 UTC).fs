...
struct Light
{
	...
	float Kc, Kl, Kq, smallLimit, bigLimit;
};
...
void main()
{
	float r = texture2D(u_texture3, v_uv + u_translation).r, g = texture2D(u_texture3, v_uv + u_translation).g, b = texture2D(u_texture3, v_uv + u_translation).b;
	float k = 1e-2;
	...
	vec4 color = texture2D(u_texture0, (v_uv + u_translation) * 100.) * b + texture2D(u_texture1, v_mini_uv + u_translation * 100.) * r + texture2D(u_texture2, v_mini_uv + u_translation/k) * g;
	...
	color = vec4(u_ambRatio * ambientalComponent + (1.0 - u_ambRatio) * (specularComponent + diffuseComponent), color.a);
